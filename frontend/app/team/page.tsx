...
export default function TeamCompatibilityPage() {
  ...
  return <main className="app-shell content-shell team-shell"><Header title="팀플 궁합" /><section className="team-content">
    {!result ? <><header className="team-intro"><img src="/images/mascot-major-clipboard.png" alt="팀원의 사주를 살펴보는 여우 마스코트" /><h1>우리 팀, 호흡이 잘 맞을까?</h1></header>
      <form className="team-form" onSubmit={submit}><div className="team-member-list">{members.map((member, index) => <fieldset className="team-member" key={member.id}><legend>팀원 {index + 1}{index === 0 ? " (나)" : ""}</legend>{members.length > 2 && index > 0 && <button className="team-remove" type="button" onClick={() => removeMember(member.id)} aria-label={`팀원 ${index + 1} 삭제`}>×</button>}<label><span>이름</span><input value={member.name} onChange={event => update(member.id, { name: event.target.value })} placeholder="이름" maxLength={12} /></label><div className="team-birth-row"><div className="team-picker-field"><span>생년월일</span><BirthDateField value={member.birthDate} calendar={member.calendarType} leap={member.leapMonth} onChange={(birthDate, calendarType, leapMonth) => update(member.id, { birthDate, calendarType, leapMonth })} /></div><div className="team-picker-field"><span>태어난 시간</span><BirthTimeField value={member.birthTime} onChange={birthTime => update(member.id, { birthTime })} /></div></div></fieldset>)}</div>
        {members.length < 5 && <button className="team-add" type="button" onClick={addMember}>+ 팀원 추가</button>}{error && <p className="team-error" role="alert">{error}</p>}<button className="primary team-submit" disabled={busy || members.some(member => !member.name.trim())}>{busy ? "사주를 맞춰보는 중…" : `${members.length}명 팀플 궁합 보기`}</button><small className="team-form-note">입력한 정보는 궁합 계산에만 사용되며 저장되지 않아요.</small></form></> : <TeamResultView result={result} members={members} onReset={() => setResult(undefined)} />}
  </section></main>;
}

function TeamResultView({ result, members, onReset }: { result: TeamResult; members: Member[]; onReset: () => void }) {
  return <section className="team-result"><p className="team-pair">{members.map(member => member.name.trim()).join(" · ")}</p><div className="team-score"><strong>{result.score}</strong><span>점</span></div><h1>{result.title}</h1><article className="team-roles"><span>사주로 본 추천 역할</span>{result.roles.map(item => <div key={item.name}><b>{item.name}</b><em>{item.role}</em></div>)}</article><article><span>팀 전체 조언</span><p>{result.tip}</p></article><section className="team-pairs"><h2>팀원별 궁합</h2>{result.pairs.map(pair => <article key={`${pair.first}-${pair.second}`}><div><b>{pair.first} × {pair.second}</b><strong>{pair.score}점</strong></div><p>{pair.summary}</p></article>)}</section><button className="team-reset" type="button" onClick={onReset}>팀원 정보 다시 입력하기</button><small>전통 명리의 오행 관계와 지지 합충을 바탕으로 보는 참고용 결과예요.</small></section>;
}
