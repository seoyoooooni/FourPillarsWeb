...
사주 원국부터 오행, 십성, 신살, 용신과 대운까지 외부 사주 계산 API 없이 백엔드에서 직접 계산합니다. 복잡한 계산 결과는 점수, 그래프와 설명으로 풀어 사용자가 쉽게 살펴볼 수 있도록 구성했습니다.
...
### 3.1 평생 사주 — 모든 개인화 기능의 기반

<p align="center"><img src="README_images/12-home-lifetime.png" width="280" alt="평생 사주 메인 메뉴" /></p>
...
### 3.2 오늘의 운세 — 원국과 오늘의 일진 비교

<p align="center"><img src="README_images/09-home-today.png" width="280" alt="오늘의 운세 메인 메뉴" /></p>
...
### 3.3 학과 추천 — 사주를 전공 탐색 지표로 변환

<p align="center"><img src="README_images/10-home-major.png" width="280" alt="학과 추천 메인 메뉴" /></p>
...
### 3.4 학업운 — 하루 한 번 카드 뽑기

<p align="center"><img src="README_images/11-home-study.png" width="280" alt="학업운 메인 메뉴" /></p>
...
<p align="center"><img src="README_images/13-home-team.png" width="280" alt="팀플 궁합 메인 메뉴" /></p>
...
모든 날짜와 절입 계산은 대한민국 표준시(`Asia/Seoul`)를 기준으로 합니다.
...
오늘의 운세 점수는 오락용 상대 지표이며 성별에 따른 점수 가중치는 사용하지 않습니다.
...
핵심 계산과 인증 흐름은 단위 테스트와 Spring 통합 테스트로 검증합니다.

- 존재하지 않는 음력 날짜와 잘못된 윤달 입력을 거부하고, 실제 윤달 날짜는 허용하는지 확인합니다.
- 입춘 전후에 연주와 월주가 변경되는지, 23시 야자시에서 다음 날 일주가 적용되는지 확인합니다.
- 지장간·신살·용신·대운 계산과 양력으로 변환한 음력 생일의 대운 계산을 검증합니다.
- 같은 사주 입력에는 동일한 학과 순위를 반환하고, 관심 분야와 학습 환경에 따라 추천이 달라지는지 확인합니다.
- 회원가입·비밀번호 암호화·로그인·보호 API 접근·리프레시 토큰 회전 및 재사용 차단·로그아웃을 통합 테스트합니다.
- 인증이 필요한 프로필 API와 프로필 생성·조회·수정 흐름을 통합 테스트합니다.
...
백엔드가 다른 주소에서 실행 중이라면 `frontend/.env.local`에 주소를 지정합니다.
